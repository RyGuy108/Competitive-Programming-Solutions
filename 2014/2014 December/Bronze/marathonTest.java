...
public class marathonTest {
    public static void main(String[] args) throws Exception {
        ...
        long bestSave = 0;
        // try skipping checkpoint i (i from 1..N-2 correspond to original indices 2..N-1)
        for (int i = 1; i <= N - 2; i++) {
            long save = manhattan(x[i-1], y[i-1], x[i], y[i])
                      + manhattan(x[i], y[i], x[i+1], y[i+1])
                      - manhattan(x[i-1], y[i-1], x[i+1], y[i+1]);
